...
contract EXRUSDPool is PoolsRef {
    ...
    function reweight() public {
        ...
        uint RBTPrice =  _getRBTPrice();

        require (oraclePrice < RBTPrice.mul(1).div(100),'can not do now');
        //1.撤出流动性
    ...
    function _checkAchieveTheGoal() internal{
        ...
        uint RBTPrice =  _getRBTPrice();
        //没达到要求
        if(oraclePrice < RBTPrice.mul(1).div(100)){
            numberOfFailures++;
    ...
    function _returnSpecifyPrice() internal{
        ...
        //todo 需要再定PEG
        uint peg = 100;
        uint256 amountRusd = _getAmountToPeg(otherReserves, rusdReserves, peg);
    ...
    /*
        @dev获取RBT挖矿价格
    */
    function _getRBTPrice() internal view returns(uint) {
        return 100;
    }
