...
import '../interface/gov/IBlockNumber.sol';
contract CityGovernanceCommittee  is CityNodeRef,CoreRef,IGovernanceCommittee,IParliament,VoteIdRef,ICityGovCommittee,IBlockNumber {
 ...
 using SafeMath for uint256;
    ...
    event cityGovernanceCreate(uint id, uint cityId, uint creationTime);
    ...
    function inNode(uint cityGovId, address sender) public view override returns (bool) {
        uint nodeId = cityNode.getUserNode(sender);
        uint  cityId = cityGovernances[cityGovId].cityId;
        return (cityNode.getNodeCityGov(nodeId) == cityId);
    }
    ...
    /*
        ...
        @params 城市hash
    */
    function newGovernanceCommittee (uint id,uint areaId) external override {
        govCount++;
        cityGovernance storage c = cityGovernances[govCount];
        c.id = govCount;
        c.creationTime = block.timestamp;
        c.cityId = id; //真实城市治理的ID
        cityByArea[id] = areaId;
        // c.proposalBlockNumber = block.timestamp;
        areaCity[areaId].push(id);
        if(areaCity[areaId].length == 10) {
            IGovernanceCommittee(areaGovernanceAddr).newGovernanceCommittee(areaId,0);
        }
        CityNodeFundManagement _cfm = new CityNodeFundManagement(address(this),address(core()),address(Rbt()));
        c.cfm = address(_cfm);
        govParliament[govCount].impeachIndex = 12;
        govParliament[govCount].voteId = IVoteId(voteIdAddress).incrVoteId();
        emit cityGovernanceCreate(c.id,id,block.timestamp);
    }

    function getAreaIdByCityId(uint id) external override view returns(uint) {
        return cityByArea[id];
    }
...
//        require(inNode(nodeId, msg.sender), 'not in node');
    ...
    */
    ...
    function endToParliament(uint id) external   {
        require(govParliament[id].votePeriod == true);
        (uint length) = govParliament[id].parliament.length();
        require(length >=15 ,'not enough people');
        _quickSort(parliamentApplyRecords[id][govParliament[id].voteId], 0, length);

        govParliament[id].votePeriod= false;
        govParliament[id].expireTime = block.timestamp + 30 days;
        cityGovernances[id].manager =  parliamentApplyRecords[id][govParliament[id].voteId][0].account;
        //多签30天到期
        for (uint i = 1; i < 13; i++) {
            govParliament[id].parliament.add(parliamentApplyRecords[id][govParliament[id].voteId][i].account);
        }
        if (length > 13) {
            for (uint i = 13; i < length; i++) {
                address user = parliamentApplyRecords[id][govParliament[id].voteId][i].account;
                IERC20 rbt = core().rbt();
                TransferHelper.safeTransferFrom(address(rbt),cityGovernances[id].cfm,msg.sender,1000 * 10 ** 18);
            }
        }
    }
